...
- **sera-mcp の `--stateless` は動かなかった**。Lambda 上で毎回 `Error POSTing to endpoint:` と空のエラーで失敗し、しかも失敗した接続の Promise をキャッシュしていたため、以降のリクエストが即座に落ちていた（応答時間が 1.9 秒 → 57ms に変わったのが手がかり）。原因は、sera-mcp が stateless 時に単一のトランスポートを使い回し、同梱の MCP SDK 1.30.0 が2回目のリクエストを拒否すること。ローカルで同じバンドルを stateless / ステートフルで動かして再現し、ステートフル起動に切り替えて解決した（Sera の Sepolia から USDC のメタデータ取得を確認）。
- **LLM は与えた範囲から「サポート範囲」を推測して断言する**。最初、残高の対象を USDC/XSGD/MYRT の3種類に固定していたところ、エージェントが「Sera が対応するのはその3種類だけ」と回答した。実際には Sera の `/tokens` に約150種類（USDT や JPYC を含む）が登録されていた。ツールの出力に「何を確認したか」（確認したトークン数、0は省略）を含め、レジストリ全体をマルチコール1回で調べる方式にした。
- **チャットの初回応答を確認**（デプロイ済み dev）: 「残高を教えて」に対し、Bedrock（Nova 2 Lite）がツールを呼び、日本語で ETH と各トークン残高を回答した。応答時間・トークン数などの計測は `TODO(実測)`。
- **見積は Lambda 上の sera-mcp 経由で取得できた**（デプロイ済み dev、パネルとチャットの両方）。ただし Sera テストネットの見積レートは、USD ステーブル同士でも 1 に近くない値（例: 0.8366）だった。テストネットの流動性・価格は実市場を反映しない可能性がある。
- **「承認」の設計をプロンプトで曖昧にすると、LLM が自前の承認フローを作る**。「チャット上の明示的な承認が必要」と書いたところ、モデルは見積を出した後に「『同意』と返してください」と要求し、`request_swap`（確認画面を出すツール）を呼ばなかった。「同意」を受けても実行するツールは無く、会話が止まった。承認はチャットの返信ではなく、確認画面のボタン + ウォレット署名であることと、見積 → `request_swap` を同じターンで呼ぶ手順を、プロンプトに明示した。修正後の挙動は `TODO(実測)`。
- **MYRT の swap は EIP-2612 permit が必要だった**。Sera の見積は、ウォレット保有分の swap で入力トークンが permit 対応だと、`permit.eip712`（署名すべき typed data 一式）を返す。当初は未対応として拒否していたが、実際に試したのが MYRT だったため対応した: 見積から permit を保存 → 確認画面で swap と permit の2回署名 → サーバーで permit 署名がユーザー自身のウォレットのものか（`recoverTypedDataAddress`）と `owner` を検証 → `permit_signature` と、サーバー保存の `deadline` を `execute_swap` に渡す。ユニットテスト済み、Sera への実送信は `TODO(実測)`。
- **会話履歴を保存していたのに、次のターンに渡していなかった**。メッセージは DynamoDB に保存していたが、リクエストごとに新しい Agent を作っており、過去の会話を読み込んでいなかったため、「OK」と返すと「おかえりなさい！」と初対面の応答になった。保存済みの会話を Strands の `messages` に渡すようにした。Bedrock は「user 始まり・user/assistant 交互」を要求するため、空の発話の除去・同じ役割の結合・先頭の assistant と末尾の user の除去・直近20件への制限を行う変換関数を作りテストした。保存しているのはテキストだけなので、見積もりIDのように後続で必要な値は、返答本文に含めるようプロンプトで指示した。
- **swap が最後まで通った**（デプロイ済み dev、Sepolia）: MYRT 10 → USDT。ウォレット残高が MYRT 10,000 → 9,990、USDT 10,000 → 10,000.699943 に変化したことをオンチェーン残高で確認した（見積もりパネルの想定は約0.66 USDT）。MYRT は EIP-2612 permit が必要なトークンで、確認画面での swap と permit の2回署名、サーバー側の permit 署名検証を経て実行された。所要時間、トランザクションハッシュ、手数料の実測は `TODO(実測)`。
- **swap が「結果確定待ち」のまま止まった**。決済状態を返す Sera の API は認証必須で、キー無しでは確定できなかった。代わりに**オンチェーンの Transfer ログで確認**する方式にした: 署名した見積の `route_params`（入力/出力トークン、最大入力額、最小受取額、受取先）と、「ウォレットの入力トークンの Transfer」「受取先への出力トークンの Transfer」の**両方**が見つかったときだけ成功にする（片方だけでは成功と見なさない）。署名期限を過ぎて一定時間、決済が見つからなければ失敗として確定する。実行済みの取引にも効くよう、承認記録の署名データから復元する。この方式は誤検知の余地（同時期の無関係な同額の Transfer）がゼロではない。実機での挙動は `TODO(実測)`。
